...
pub fn media_player(_props: &MediaPlayerProps) -> Html {
    const DEFAULT_VOLUME: f64 = 0.3;
    let audio_ref = use_node_ref();
    let playlist = use_state(|| {
        vec![
    ...
    // Update audio on track change
    {
        ...
        use_effect_with(
            (audio_ref.clone(), current_track.clone()),
